...
class User extends ActiveRecord
{
	...
	public function rules(){
		return [
			...
			['username','unique','message'=>'用户名已经被注册','on'=>['reg','regbymail','qqreg']],
	...
	public function validatePass(){
		if (!$this->hasErrors()) {
			...
			$data = self::find()->where($loginname.' = :loginname and userpass = :pass',[':loginname'=>$this->loginname,':pass'=>md5($this->userpass
				)])->one();
			if (is_null($data)) {
